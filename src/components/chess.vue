<template>
  <div class="main" v-if="ifstart">
    ...
    <transition-group tag="div" class="chess" name="animated">
      ...
      <!--<span v-for="item in chess.backItem" class="back" :class="{ 'notClick': item.notClick}"-->
            <!--@click="select(item.num,item.notClick)">-->
			<!--{{ item.message }}-->
		  <!--</span>-->
    </transition-group>
    <div class="bottom">
    ...
    <div class="back">
      <ul>
        <li v-for="item in backItems" :class="{ 'notClick': item.notClick}"
            :style="{left:item.left+'px',top:item.top+'px',position: 'absolute'}"
            @click="back(item.num,item.notClick)">
          {{ item.message }}
        </li>
      </ul>
    </div>
  ...
  export default {
    ...
    data() {
      return {
        ...
        backItems:[
          {message: '撤销',num: 1, notClick: false,left:450,top:250},
          {message: '悔棋',num: 2, notClick: false,left:450,top:300},
        ],
        chess: {
          blackItem: [
            {message: '车', pos: [{x: 0, y: 0, active: true}]},
            {message: '马', pos: [{x: 1, y: 0, active: true}]},
            {message: '象', pos: [{x: 2, y: 0, active: true}]},
            ...
            {message: '炮', pos: [{x: 1, y: 2, active: true}]},
            {message: '炮', pos: [{x: 7, y: 2, active: true}]},
            {message: '卒', pos: [{x: 0, y: 3, active: true}]},
            {message: '卒', pos: [{x: 2, y: 3, active: true}]},
    ...
    methods: {
      ding: function (item) {
        ...
        this.backItems = [
          {message: '撤销',num: 1, notClick: true,left:450,top:250},
          {message: '悔棋',num: 2, notClick: true,left:450,top:300},
        ]
      },
      select: function (message, notClick) {   //选子按钮传入  交由rule 判断后  再经过step1／2／3／4 执行下一步按钮的渲染
        if (!notClick) {
          let color;
          if (this.step % 2 == 0) {
            color = 'black';
      ...
      back : function (num,notClick) {
        ...
        if (!notClick) {
          ...
          if (num == 2) {  //悔棋
            ...
            let backRecord = this.backRecord.pop();
            console.log(backRecord)
            let pos = this.chess[backRecord.color+'Item'][backRecord.index].pos.pop();
            console.log(pos)
            console.log(this.chess[backRecord.color+'Item'][backRecord.index].pos)
            if(backRecord.chiIndex >=0){
              let chiColor = backRecord.color == 'black' ? 'red':'black';
              this.chess[chiColor+'Item'][backRecord.index].pos.pop();
            }
            if(this.step>this.dingStep){
              this.step --;
              let color;
              if (this.step % 2 == 0) {
                color = 'black';
      ...
      step2: function (color, ifqh, message) {
        if (!!ifqh) {
          ...
          this.selectItems.forEach(function (item) {
            if (!~message.indexOf(item.message)) {
              console.log("step2-true-373", item.notClick)
              item.notClick = true;
            }
          })
        } else {
          ...
          this.selectItems.forEach(function (item, index) {
            item.notClick = !~message.indexOf(index)
          })
      ...
      step3: function (color, chessIndex) {
        ...
        self.pos = {x:pos.x,y:pos.y,active:true};
        if (chessMessage == '象' || chessMessage == '相' || chessMessage == '士' || chessMessage == '仕' || chessMessage == '马' || chessMessage == '馬') {
          ...
          if (chessMessage == '象' || chessMessage == '相') {
            let poses = [{x: pos.x - 2, y: pos.y - 2}, {x: pos.x + 2, y: pos.y + 2}, {
              x: pos.x + 2,
              y: pos.y - 2
            }, {x: pos.x - 2, y: pos.y + 2}]
            self.posesFilter = [];
            if (color == 'black') {
              ...
              self.posesFilter = poses.filter(function (item) {
                return item.x >= 0 && item.x <= 8 && item.y >= 5 && item.y <= 9 && self.getChessItemByPos(color, item.x, item.y) < 0 && self.getChessItemByPos(color, (item.x + pos.x) / 2, (item.y + pos.y) / 2) < 0;
              })
            }
      ...
      action: function (color, record) {
        ...
        if (chessMessage == '象' || chessMessage == '相' || chessMessage == '士' || chessMessage == '仕' || chessMessage == '马' || chessMessage == '馬') {
          ...
          if (chiIndex >= 0) {
            ...
            self.chess[chiColor + 'Item'][chiIndex].pos.push({x: chiPos.x, y: chiPos.y, active: false})
          }
        ...
        this.backRecord.push({color:color,index : self.chessIndex,chiIndex:chiIndex})
